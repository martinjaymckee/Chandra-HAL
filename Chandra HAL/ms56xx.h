...
class MS5637
{
  ...
  public:
    ...
    bool init(const uint32_t _f_i2c=400000) {// TODO: THIS FUNCTION (AND LATER THE I2C INIT FUNCTION AS WELL, NEED TO TAKE A FREQUENCY QUANTITY RATHER THAN JUST A NUMBER)
      ...
      // TODO: CHANGE THE TIMER IMPLEMENTATION... THIS IS AN ANNOYING REQUIREMENT
      //  OF THE TIMER OBJECT AT THIS TIME.  THE RUN METHOD IS REALLY PROBABLY NOT
      //  NECESSARY AS IT WAS IMPLEMENTED TO TRY TO TAKE CARE OF THE RETRIGGERABLE
      //  TIMER CASE.  IN THE END, THE TIMER NEEDS TO BE OPTIMIZED AND I BELIEVE
      //  THAT 0THIS IS UNNECESSARY.
      adc_timer_.run();
      pressure_cfg_.timer.run();
      temperature_cfg_.timer.run();
    ...
    bool reset() {
      ...
      // AND TOGGLE THE SCL LINE SEVERAL TIMES BEFORE SENDING THE COMMAND...
      sendCMD(0x1E);
