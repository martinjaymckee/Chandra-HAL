...
\usepackage{mdframed}
\usepackage[inner=0.75in, outer=1.75in, top=0.75in, bottom=0.75in, marginparwidth=1.375in, marginparsep=0.1875in]{geometry} % to change the page dimensions
...
\newcommand{\sidenote}[1]{\marginpar{\scriptsize{#1}}}
...
\subsection{Units}
The units system supports dimensional analysis with the full SI system of base units extended with planar angles.  In addition, conversion between values are done on an as needed basis, only when operating on values with different units, or assigning to a value with different units.  Quantities may be created by either directly calling a constructor or using the UDL suffixes defined in the library.  Currently the suffixes are in the form \mintinline{cpp}{__yy__} where \emph{yy} is the standard SI unit name (or an ASCII encoding that approximates it).  For instance, meters use the suffix \mintinline{cpp}{__m__}, while mega-electronvolts use the suffix \mintinline{cpp}{__Mev__} and feet \mintinline{cpp}{__ft__}.

\subsection{Statistics}
The built-in statistics packages are minimal in Chandra and are focused, primarily, on supporting exploratory data analysis to allow for characterization of systems.  The statistics functionality can be found in \mintinline{cpp}{namespace chandra::math::statistics}.  There are currently two major statistical components, classes to capture rolling statistics, and a NRV (Normal Random Variable) value type.
...
\begin{minted}{cpp}
	...
	class Matrix;	
\end{minted}
where the value type defines a single value in the matrix and the \mintinline{cpp}{Rows} and \mintinline{cpp}{Columns} parameters define the shape of the matrix.
...
\end{minted}
we explore the available constructors.  To begin with, the default constructor, \sidenote{The zero-argument constructor is defaulted as is the copy-constructor.}
\begin{minted}{cpp}
	 matrix_t m;	
\end{minted}
simply reserves the matrix memory for \mintinline{cpp}{m}.  It does not initialize that memory.  As a result, a default constructed matrix should be treated as containing random values.  If known values are required, then another construction method with explicit initialization should be used instead.   
...
The final basic constructor is a converting constructor to allow conversion of size-matched matricies with different value types.  Here, the \mintinline{cpp}{Rows} and \mintinline{cpp}{Columns} parameters come from the type that is being constructed while \mintinline{cpp}{V} is the type of the matrix to construct from.
\begin{minted}{cpp}
	template<class V>
	constexpr Matrix<const Matrix<V, Rows, Columns>& _other);
\end{minted}
Internally, the converting constructor uses a \mintinline{cpp}{static_assert} to check that the source datatype is convertible to the destination datatype.
...
\end{minted}
The first of these will fill the matrix with an arbitrary value.  For instance, with our previously defined 3x3 matrix type \mintinline{cpp}{matrix_t},
\begin{minted}{cpp}
...
\end{align}
As the names imply, \mintinline{cpp}{Zeros} and \mintinline{cpp}{Ones} result in matricies filled with the value 0 and 1 respectively.
...
\begin{tabular}{cg}
	\rowcolor[gray]{0.8}\textbf{Type} & \textbf{Description}\\ 
	\mintinline{cpp}{value_t} & The passed in value type\\
	\mintinline{cpp}{mean_t} & The type used for the mean, typically the same as the base value type\\
	\mintinline{cpp}{standard_deviation_t} & The type used for standard deviation, typically the same as the base value type\\
	\mintinline{cpp}{variance_t} & The type used for variance, generated as \mintinline{cpp}{decltype(standard_deviation_t() * standard_deviation_t())}\\
\end{tabular} 
