...
\subsection{Units}
The units system supports dimensional analysis with the full SI system of base units extended with planar angles.  In addition, conversion between values are done on an as needed basis, only when operating on values with different units, or assigning to a value with different units.  Quantities may be created by either directly calling a constructor or using the UDL suffixes defined in the library.  Currently the suffixes are in the form \mintinline{cpp}{_yy_} where \emph{yy} is the standard SI unit name (or an ASCII encoding that approximates it).  For instance, meters use the suffix \mintinline{cpp}{_m_}, while mega-electronvolts use the suffix \mintinline{cpp}{_Mev_} and feet \mintinline{cpp}{_ft_}.
