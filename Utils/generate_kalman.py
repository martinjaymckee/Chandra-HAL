...
def dependencySymbols(expressions, subexpressions, dependency_dict=None):
    ...
    for expr in expressions:
        ...
        for s in expr_symbols:
            if s in subexpr_map and s not in symbols:
                symbols.add(s)
                dependencies = set()
                if dependency_dict is not None and s in dependency_dict.keys():
...
if __name__ == '__main__':
    ...
    if definition.Q is None:
        Q = kfg.generateDefaultProcessCovariance(definition.X, F)
